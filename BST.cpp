...
struct BST
{
    ...
    void modificar(BST *tree, Node *no, int versao, string campo, T valor)
    {
        ...
        if (no->mods.empty())
        {
            ...
            { // adicionei esse "::value" para evitar erro de compilação, e está correto
                modifica.modPointer = valor;
            }
            else
            {
                modifica.modChave = valor;
            }

            no->mods.resize(1);               // garante que haja espaço para mods[0]
    ...
    void aplicaMods(Node *atual, Node *temp, int version)
    {
        ...
        if ((atual->mods.empty()) || (atual->mods[0].ver > version))
            return;
    ...
    */
    ...
    void transplantar(BST *tree, Node* u, Node* v, int versao) 
    {
        ...
        { // se u for raiz, então v também será raiz
            ...
            if (v != nullptr)
            {
                modificar(tree, v, versao + 1, "pai", nullptr); // v não tem pai, pois é raiz
            }
        }
        else
        {
            ...
            if (tempPai.esq == u)
            {
                modificar(tree, tempU.pai, versao + 1, "esq", v); // se u for filho esquerdo de pai, então v também será filho esquerdo
            }
            else
            {
                modificar(tree, tempU.pai, versao + 1, "dir", v); // se u for filho direito de pai, então v também será filho direito
            }

            if (v != nullptr)
            {
                modificar(tree, v, versao + 1, "pai", tempU.pai); // v passa a ser filho de pai
            }
        }
    }
    ...
    void remover(BST *tree, int chave, int versao) {
        ...
        if(tempAlvo.esq == nullptr) {
            ...
            if (tempSucessor.pai != alvo) {
                ...
                modificar(tree, sucessor, versao, "dir", tempAlvo.dir); // atualiza o filho direito do sucessor
                modificar(tree, sucessor, versao, "pai", tempAlvo.pai); // atualiza o pai do sucessor
            }

            transplantar(tree, alvo, sucessor, versao); // transplanta o nó alvo pelo sucessor
            modificar(tree, sucessor, versao, "esq", tempAlvo.esq); // atualiza o filho esquerdo do sucessor
            modificar(tree, tempAlvo.esq, versao, "pai", sucessor); // atualiza a chave do sucessor
        }
